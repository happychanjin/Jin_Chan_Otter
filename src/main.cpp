...
int main() {
	...
	{  
		...
		std::cout<<shader2->LoadShaderPartFromFile("shaders/Water.vert.glsl", GL_VERTEX_SHADER)<<std::endl;
		std::cout << shader2->LoadShaderPartFromFile("shaders/Water.frag.glsl", GL_FRAGMENT_SHADER)<<std::endl;
		shader2->Link();
		...
		reflectiveShader->LoadShaderPartFromFile("shaders/frag_blinn_phong.glsl", GL_FRAGMENT_SHADER);
		...
		int		  ToonBands = 1;
		float	  edgeAdjust = 0.369f;
		glm::vec3 ambientCol = glm::vec3(1.0f);
		float     ambientPow = 0.1f;
		float     lightLinearFalloff = 0.09f;
		float     lightQuadraticFalloff = 0.032f;

		glm::vec3 heightCutoffs = glm::vec3(0.299f, 0.531f, 0.829f);
		glm::vec3 interpolateFactors = glm::vec3(0.05f, 0.08f, 0.08f);
		...
		shader->SetUniform("u_bands", ToonBands);
		shader->SetUniform("u_edgeAdjust", edgeAdjust);  
		...
		shader2->SetUniform("u_HeightCutoffs", heightCutoffs);
		shader2->SetUniform("u_InterpolateFactors", interpolateFactors);
		...
		reflectiveShader->SetUniform("u_Shininess", 8.0f);
		reflectiveShader->SetUniform("u_edgeAdjust", edgeAdjust);
		reflectiveShader->SetUniform("u_bands", ToonBands);
		...
		imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::CollapsingHeader("Light Level Lighting Settings"))
			{
				...
				if (ImGui::SliderInt("Toon", &ToonBands, 0.0f, 20.0f)) {
					shader->SetUniform("u_bands", ToonBands);
					reflectiveShader->SetUniform("u_bands", ToonBands);
				}
				if (ImGui::SliderFloat("edge Adjust", &edgeAdjust, 0.0f, 1.0f)) {
					shader->SetUniform("u_edgeAdjust", edgeAdjust);
					reflectiveShader->SetUniform("u_edgeAdjust", edgeAdjust);
				}
			} 
			...
			if (ImGui::Button("No Lighting")) {
				...
				shader->SetUniform("u_bands", 1);
				shader->SetUniform("u_edgeAdjust", 0.0f);

				reflectiveShader->SetUniform("u_AmbientLightStrength", 0.0f);
				reflectiveShader->SetUniform("u_SpecularLightStrength", 0.0f);
				reflectiveShader->SetUniform("u_bands", 0);
				reflectiveShader->SetUniform("u_edgeAdjust", 0.0f);
			}
			if (ImGui::Button("ambient lighting only ")) {
				...
				reflectiveShader->SetUniform("u_AmbientLightStrength", 1.0f);
				reflectiveShader->SetUniform("u_SpecularLightStrength", 0.0f);
				reflectiveShader->SetUniform("u_bands", 2);
				reflectiveShader->SetUniform("u_edgeAdjust", 0.0f);
			}
			if (ImGui::Button("specular lighting only ")) {
				shader->SetUniform("u_AmbientLightStrength", 0.0f);
				shader->SetUniform("u_SpecularLightStrength", 1.0f);
				shader->SetUniform("u_bands", 2);
				shader->SetUniform("u_edgeAdjust", 0.0f);

				reflectiveShader->SetUniform("u_AmbientLightStrength", 0.0f);
				reflectiveShader->SetUniform("u_SpecularLightStrength", 1.0f);
				reflectiveShader->SetUniform("u_bands", 2);
				reflectiveShader->SetUniform("u_edgeAdjust", 0.0f);
			}
			if (ImGui::Button("Ambient + specular")) {
				shader->SetUniform("u_AmbientLightStrength", 0.235f);
				shader->SetUniform("u_SpecularLightStrength", 1.0f);
				shader->SetUniform("u_bands", 2);
				shader->SetUniform("u_edgeAdjust", 0.0f);

				reflectiveShader->SetUniform("u_AmbientLightStrength", 0.235f);
				reflectiveShader->SetUniform("u_SpecularLightStrength", 1.0f);
				reflectiveShader->SetUniform("u_bands", 2);
				reflectiveShader->SetUniform("u_edgeAdjust", 0.0f);
			}
			if (ImGui::Button("Ambient + specular + Toon")) {
				shader->SetUniform("u_AmbientLightStrength", 0.206f);
				shader->SetUniform("u_SpecularLightStrength", 0.261f);
				shader->SetUniform("u_bands", 1);
				shader->SetUniform("u_edgeAdjust", 0.555f);

				reflectiveShader->SetUniform("u_AmbientLightStrength", 0.348f);
				reflectiveShader->SetUniform("u_SpecularLightStrength", 0.147f);
				reflectiveShader->SetUniform("u_bands", 1);
				reflectiveShader->SetUniform("u_edgeAdjust", 0.391f);
			}
		...
		Texture2D::sptr heightMap = Texture2D::LoadFromFile("images/heightmap.bmp");
		...
		material0->Set("myTextureSampler", heightMap); 
	...
	//TextureCubeMap::sptr environmentMap = TextureCubeMap::LoadFromImages("images/cubemaps/skybox/sample.jpg");
		...
		GameObject terrain = scene->CreateEntity("Terrain");
		{
			...
			terrain.get<Transform>().SetLocalPosition(-0.0f, 0.0f, 2.0f).SetLocalScale(glm::vec3(10));
		...
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/plane.obj");
			Water.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material2);
			Water.get<Transform>().SetLocalPosition(-0.0f, 0.0f, 4.5f).SetLocalScale(glm::vec3(10));
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(Water);
		}

		GameObject obj = scene->CreateEntity("Player1");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Player.obj");
			obj.emplace<RendererComponent>().SetMesh(vao).SetMaterial(reflectiveMat);
			obj.get<Transform>().SetLocalPosition(2.0f, 0.0f, 10.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj);
		}
		GameObject obj2 = scene->CreateEntity("Player2");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Player2.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(reflectiveMat);
			obj2.get<Transform>().SetLocalPosition(4.0f, 0.0f, 14.0f);
		...
		while (!glfwWindowShouldClose(window)) {
			...
			if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
				obj.get<Transform>().MoveLocal(0.0f, 0.1f, 0.0f);
			}
			if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) {
				obj.get<Transform>().MoveLocal(0.0f, -0.1f, 0.0f);
			}
			if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) {
				obj.get<Transform>().MoveLocal(-0.1f, 0.0f, 0.0f);
			}
			if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) {
				obj.get<Transform>().MoveLocal(0.1f, 0.0f, 0.0f); 
			}
			if (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS) {
				obj.get<Transform>().MoveLocal(0.0f, 0.0f, -0.1f);
			}
