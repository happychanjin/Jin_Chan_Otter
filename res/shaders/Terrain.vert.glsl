...
layout(location = 5) out vec3 outVert;
...
void main() {
	...
	vert.z = texture(myTextureSampler,inUV).r;
	outVert = vert;

	//gl_Position =   u_ModelViewProjection * vec4(inPosition.x,inPosition.y,sin(inPosition.x/inPosition.y), 0.95);
	gl_Position =   u_ModelViewProjection * vec4(vert, 0.95);
