...
layout(location = 5) in vec3 outVert;
...
void main() {
	...
	//outline effect
	float edge = (dot(viewDir, N) < u_edgeAdjust) ? 0.0 : 1.0;
	...
	vec3 result = (
		...
		(ambient + (diffuse* attenuation) + specular)  // light factors from our single light
		) * inColor * textureColor.rgb*scaleFactor ; // Object color
		
	frag_color = vec4(result*edge, 1.0);
	//frag_color = vec4(1.0,0.0,0.0, 0.8);
}